...
public class FeedSetupActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public static class SuggestedFilterCell extends FrameLayout {
        ...
        private FeedFilter suggestedFilter;
        ...
        public void setFilter(FeedFilter filter, boolean divider) {
            needDivider = divider;
        ...
        public FeedFilter getSuggestedFilter() {
    ...
    public static class FilterCell extends FrameLayout {
        ...
        public void setFilter(FeedFilter filter, boolean divider) {
            ...
            if (!filter.feedDialogsId.isEmpty()) {
                if (info.length() != 0) {
                    info.append(", ");
                }
                info.append(LocaleController.formatPluralString("Exception", filter.feedDialogsId.size()));
            }
            if (info.length() == 0) {
                info.append(LocaleController.getString("FilterNoChats", R.string.FilterNoChats));
            }

            textView.setText(Emoji.replaceEmoji(filter.title, textView.getPaint().getFontMetricsInt(), AndroidUtilities.dp(20), false));
    ...
    public boolean onFragmentCreate() {
        FeedManager.getInstance(currentAccount).loadFeedFilters();
        updateRows(true);
        getNotificationCenter().addObserver(this, NotificationCenter.feedFiltersUpdated);
        getNotificationCenter().addObserver(this, NotificationCenter.suggestedFiltersLoaded);
        if (FeedManager.getInstance(currentAccount).suggestedFilters.isEmpty()) {
            FeedManager.getInstance(currentAccount).loadSuggestedFilters();
        }
    ...
    private void updateRows(boolean notify) {
        ...
        ArrayList<FeedFilter> suggestedFilters = FeedManager.getInstance(currentAccount).suggestedFilters;
        rowCount = 0;
        filterHelpRow = rowCount++;
        int count =FeedManager.getInstance(currentAccount).feedFilters.size();
    ...
    public void onFragmentDestroy() {
        getNotificationCenter().removeObserver(this, NotificationCenter.feedFiltersUpdated);
        getNotificationCenter().removeObserver(this, NotificationCenter.suggestedFiltersLoaded);
        if (orderChanged) {
            getNotificationCenter().postNotificationName(NotificationCenter.feedFiltersUpdated);
            FeedManager.getInstance(currentAccount).saveFeedFiltersOrder();
        }
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle("Feed Filters");
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
             if (position >= filtersStartRow && position < filtersEndRow) {
                presentFragment(new FeedCreateActivity(FeedManager.getInstance(currentAccount).feedFilters.get(position - filtersStartRow),FeedManager.getInstance(currentAccount).feedFilters.get(position - filtersStartRow).feedDialogsId));
            } else if (position == createFilterRow) {
                presentFragment(new FeedCreateActivity());
            }
        });
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == NotificationCenter.feedFiltersUpdated) {
            if (ignoreUpdates) {
                return;
            }
            updateRows(true);
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
            ...
            switch (viewType) {
                ...
                case 2:
                    ...
                    filterCell.setOnOptionsClick(v -> {
                        ...
                        FeedFilter filter = cell.getCurrentFilter();
                        AlertDialog.Builder builder1 = new AlertDialog.Builder(getParentActivity());
                        TextPaint paint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
                        paint.setTextSize(AndroidUtilities.dp(20));
                        builder1.setTitle(Emoji.replaceEmoji(filter.title, paint.getFontMetricsInt(), AndroidUtilities.dp(20), false));
                        ...
                        builder1.setItems(items, icons, (dialog, which) -> {
                            if (which == 0) {
                                presentFragment(new FeedCreateActivity(filter));
                            } else if (which == 1) {
                                ...
                                builder.setPositiveButton(LocaleController.getString("Delete", R.string.Delete), (dialog2, which2) -> {
                                    ...
                                    AndroidUtilities.runOnUIThread(new Runnable() {
                                        ...
                                        public void run() {
                                            ...
                                            int idx = FeedManager.getInstance(currentAccount).feedFilters.indexOf(filter);
                                            if (idx >= 0) {
                                                idx += filtersStartRow;
                                            }
                                            ignoreUpdates = true;
                                            FeedManager.getInstance(currentAccount).removeFilter(filter);
                                            FeedManager.getInstance(currentAccount).deleteFeedFilter(filter);
                                            ignoreUpdates = false;
                                ...
                                presentFragment(new FeedFragment(filter));
                            }
                        });
                ...
                default:
                    ...
                    suggestedFilterCell.setAddOnClickListener(v -> {
                        FeedFilter suggested = suggestedFilterCell.getSuggestedFilter();
                        FeedFilter filter = new FeedFilter();
                        filter.title = suggested.title;
                        filter.id = 2;
                        while (FeedManager.getInstance(currentAccount).feedFiltersById.get(filter.id) != null) {
                            filter.id++;
                        }

                        ignoreUpdates = true;
                        FeedCreateActivity.saveFilter(filter, filter.emoticon, filter.title, filter.feedDialogsId, true, true, true, false, FeedSetupActivity.this, () -> {
            ...
            switch (holder.getItemViewType()) {
                case 0: {
                    ...
                    if (position == filtersHeaderRow) {
                        headerCell.setText("Feed Filters");
                ...
                case 2: {
                    ...
                    filterCell.setFilter(FeedManager.getInstance(currentAccount).feedFilters.get(position - filtersStartRow), true);
                    break;
                }
                case 3: {
                ...
                case 5: {
                    ...
                    filterCell.setFilter(FeedManager.getInstance(currentAccount).suggestedFilters.get(position - recommendedStartRow), recommendedStartRow != recommendedEndRow - 1);
                    break;
                }
                case 6: {
        ...
        public void swapElements(int fromIndex, int toIndex) {
            ...
            ArrayList<FeedFilter> filters = FeedManager.getInstance(currentAccount).feedFilters;
            FeedFilter filter1 = filters.get(idx1);
            FeedFilter filter2 = filters.get(idx2);
